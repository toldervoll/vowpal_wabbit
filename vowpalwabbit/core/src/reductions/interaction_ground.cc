...
{
public:
  ik_stack_builder(VW::LEARNER::base_learner* ftrl_coin) : _ik_base(ftrl_coin)
  {
...
void learn(interaction_ground& igl, multi_learner& base, VW::multi_ex& ec_seq)
{
...
base_learner* VW::reductions::interaction_ground_setup(VW::setup_base_i& stack_builder)
{
  ...
  // 2 weight vectors needed. 0 for ik, 1 for pi
  size_t problem_multiplier = 2;
  ...
  ld->ik_learner = as_singleline(ik_builder->setup_base_learner());
